...
const deletePost = (postDraft, setPostDraft, postingsDataArray, setPostingsDataArray, currPostIndex, setShowMainModal, creatingPostFlag) => {
  ...
  if (!creatingPostFlag && (postingsDataArray?.[currPostIndex]?._id)) {   
    ...
    console.log("deletePost.js archiving: postDraft=", postDraft);

    deletePostFromDB(postDraft, setPostDraft);
